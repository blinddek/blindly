...
function ImportTab() {
  ...
  useEffect(() => {
    async function load() {
      const [supplierData] = await Promise.all([getSuppliers()]);
      setSuppliers(supplierData);
      if (supplierData.length > 0) {
        setSelectedSupplier(supplierData[0].slug);
      }
  ...
  function updateOverride(sheetName: string, update: Partial<SheetMappingOverride>) {
    setOverrides((prev) => {
      ...
      if (current) {
        next.set(sheetName, { ...current, ...update });
      }
      return next;
    });
  }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="pt-6 space-y-4">
          ...
          <Button
            onClick={handleParse}
            disabled={!file || !selectedSupplier || parsing}
          >
      ...
      <CardContent className="pt-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Badge variant="secondary" className="gap-1">
            {result.extras_synced} Extras
          </Badge>
          <Badge variant="secondary" className="gap-1">
            {result.mechanisms_synced} Mechanisms
          </Badge>
          <Badge variant="secondary" className="gap-1">
            {result.motors_synced} Motors
          </Badge>
          <Badge variant="secondary" className="gap-1">
            {result.products_synced} Products Synced
          </Badge>
        </div>
...
function MappingsTab() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setSuppliers(data);
      if (data.length > 0) {
        setSelectedSupplier(data[0].slug);
      }
      setLoading(false);
    }
    load();
          ...
          <CardContent className="py-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap gap-1.5">
              <Badge variant="secondary" className="text-xs">
                {entry.sheets_processed} sheets
              </Badge>
              <Badge variant="default" className="text-xs">
                {entry.prices_created} prices
              </Badge>
